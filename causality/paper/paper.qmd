...
Este artículo analiza, con el panel longitudinal ELRI (2018–2023), cómo el estallido social y el estado de excepción en la Macrozona Sur se asociaron con cambios en la justificación de la violencia entre personas indígenas y no indígenas en el sur de Chile. Estudiamos los efectos diferenciales del estallido (2019) y del estado de excepción prolongado (2021–2023) mediante un diseño de diferencias en diferencias. El estallido elevó la justificación de la violencia de cambio social para toda la muestra (`r fmt_beta_p(paper$b_ola3_resg, paper$p_ola3_resg)`), sin efecto diferencial por territorio ni identidad étnica — confirmando su carácter de fenómeno nacional. El decreto produjo un efecto diferencial significativo entre personas indígenas residentes en la zona de excepción: mayor justificación de la resistencia colectiva (`r paper$tau4_resg$beta_p`). Para la represión estatal, el efecto directo no alcanza significancia en el Modelo C (`r paper$tau4_ctrl$beta_p`); el decreto sí mejoró la JP ingroup (`r fmt_beta_p(paper$b_T_M_ingroup, paper$p_T_M_ingroup)`), evidencia de regularización procedimental. Un ajuste por JP pre-decreto atenúa τ₄ sobre control (~`r round(paper$med_ctrl_pct, 0)`%) y amplifica τ₄ sobre resistencia (~`r round(paper$sup_resg_pct, 0)`%), sin identificar mediación causal. La coerción institucionalizada no produjo un backlash unidireccional sino una reconfiguración normativa dual.
...
## Reconfiguración normativa dual: mecanismos simultáneos {#sec-mecanismo-dual}

La coerción prolongada e institucionalizada puede alterar actitudes por dos vías simultáneas: un efecto directo de la exposición a la represión y un efecto indirecto vía la percepción de justicia procedimental hacia el propio grupo (ingroup). La regularización del trato institucional — predecibilidad, no benevolencia — puede legitimar la represión estatal sin suprimir la resistencia territorial. @fig-modelo-conceptual resume este modelo conceptual (esquema teórico; no SEM ni mediación causal identificada).

```{r}
#| label: fig-modelo-conceptual
#| fig-cap: "Figura 1. Modelo conceptual: decreto → JP ingroup (regularización, Paso 1) coexistiendo con efecto directo sobre resistencia. El ajuste por JP pre-decreto (Paso 3) es diagnóstico, no path causal. No es un modelo de ecuaciones estructurales."
#| fig-width: 10
#| fig-height: 5
#| out-width: "100%"
...
Se utilizan las olas 2 (2018), 3 (dic 2020–may 2021) y 4 (2023) en el análisis principal. El panel exige observación en las cuatro olas ELRI (2016–2023) para el criterio de balance; la ola 1 (2016) se usa solo en el placebo pretratamiento. La muestra analítica incluye `r paper$n_indi_ola2` personas que se autoidentifican como pertenecientes a algún pueblo originario (`a1` ∈ 1–11) y `r paper$n_noindi_ola2` personas no indígenas (`a1` = 12) en baseline (ola 2). La pertenencia étnica se construye a partir de la autoidentificación en ola 1 y se mantiene fija a lo largo del panel para evitar endogeneidad. El panel balanceado comprende `r scales::label_number(big.mark = ".", decimal.mark = ",")(paper$n_individuos)` individuos presentes en las tres olas analíticas (`r scales::label_number(big.mark = ".", decimal.mark = ",")(paper$n_persona_olas)` persona-olas).
...
Los índices compuestos de dos ítems (violencia de cambio social; identidad étnica, *r* ≈ 0,82) muestran consistencia interna adecuada. La justificación de violencia de cambio social presenta α = 0,746 (*r* = 0,602 entre d4_2 y d4_3). Los ítems únicos — represión estatal (d3_1), justicia procedimental ingroup y outgroup — no requieren estimación de consistencia interna (@tbl-apendice-alpha).
...
Se estiman modelos multinivel con intercepto aleatorio por individuo (`folio`) como especificación titular. El coeficiente de interés es la triple interacción **Ola 4 × indígena × zona de excepción** ($\tau_4$). El mecanismo descompone JP ingroup/outgroup (Paso 1 post-decreto; Paso 3 = ajuste por JP pre-decreto). Robustez: balance, PSM con caliper, IPW con `feols` + SE cluster comunal, FE individuales, DRDID (submuestra indígena), placebo (@sec-apendice-a4–A5) y sensibilidad (@sec-apendice-a7); plebiscito 2022 en @sec-apendice-a10 (asociacional).
...
Un patrón relevante emerge en baseline (ola 2): los indígenas residentes en la zona de excepción muestran menor justificación de la violencia de cambio social que los indígenas fuera de ella (media = `r fmt_es(paper$desc_indi_cerca_cambio, 2)` vs. `r fmt_es(paper$desc_indi_lejos_cambio, 2)`). Esta divergencia inicial en dirección opuesta al efecto observado en ola 4 es compatible con el supuesto de tendencias paralelas — si el grupo tratado ya hubiera sido sistemáticamente más proclive a la resistencia antes del decreto, el DiD estaría sobreestimando el efecto; lo que observamos es lo contrario.
...
modelsummary(
  ...
  notes = paste0(
    ...
    "N = ", paper$n_modelo_C, " por VD (listwise deletion en controles)."
  ),
  output = ms_out()
) |> style_ms()
```

## Bloque 3 — Mecanismo: regularización y ajuste procedimental {#sec-res-mecanismo}

El análisis usa la **misma escala continua 1–5** que el Modelo C (`idx_vio_control`, `idx_vio_resguardo`). El Paso 1 estima el efecto del decreto sobre justicia procedimental en ola 4. El Paso 3 compara $\tau_4$ con y sin JP rezagada (ola 3): es un **ajuste por baseline procedimental pre-decreto**, no mediación causal — el porcentaje de atenuación no identifica el path decreto→M→Y. @fig-esquema-mediacion resume el esquema; @fig-justproc muestra las trayectorias ingroup/outgroup.

```{r}
#| label: fig-esquema-mediacion
#| fig-cap: "Esquema de mecanismo. Paso 1: el decreto mejora JP ingroup (`r fmt_beta_p(paper$b_T_M_ingroup, paper$p_T_M_ingroup)`). Paso 3: ajuste por JP rezagada (pre-decreto); el Δβ (~`r round(paper$med_ctrl_pct, 0)`% / ~`r round(paper$sup_resg_pct, 0)`%) no identifica mediación causal. Exploratorio misma-ola en apéndice."
#| fig-width: 10
#| fig-height: 5
#| out-width: "100%"
...
|----|------------|-------|-------|--------|---------|
| Vio. control | Ingroup lag | `r fmt_es(paper$med_ctrl_sin$estimate, 3)` | `r fmt_es(paper$med_ctrl_med$estimate, 3)` | **`r fmt_es(paper$ate_ctrl, 0)`%** | Atenuación al ajustar |
| Vio. control | Brecha lag | `r fmt_es(paper$med_ctrl_sin$estimate, 3)` | `r fmt_es(paper$med_ctrl_brecha_med$estimate, 3)` | `r fmt_es(paper$med_ctrl_brecha_pct, 0)`% | Atenuación al ajustar |
| Vio. resguardo | Ingroup lag | `r fmt_es(paper$med_resg_sin$estimate, 3)` | `r fmt_es(paper$med_resg_med$estimate, 3)` | **`r fmt_es(paper$ate_resg, 0)`%** | Amplificación (supresión) |
| Vio. resguardo | Brecha lag | `r fmt_es(paper$med_resg_sin$estimate, 3)` | `r fmt_es(paper$med_resg_brecha_med$estimate, 3)` | **`r fmt_es(-paper$med_resg_brecha_pct, 0)`%** | Amplificación (supresión) |

: Ajuste DiD por JP rezagada (escala continua 1–5) {#tbl-mediacion-inout}
...
**Interpretación:** El Paso 1 muestra que el decreto mejora la JP ingroup (`r fmt_beta_p(paper$b_T_M_ingroup, paper$p_T_M_ingroup)`). El Paso 3 indica que condicionar por JP pre-decreto atenúa τ₄ sobre control (~`r round(paper$med_ctrl_pct, 0)`%) y amplifica τ₄ sobre cambio social (~`r round(paper$sup_resg_pct, 0)`%): patrones compatibles con un rol de la JP como covariable de baseline, no con mediación post-tratamiento identificada. El hallazgo titular permanece el efecto directo sobre la resistencia; la regularización procedimental (Paso 1) es evidencia separada de reconfiguración institucional.

@fig-mediacion compara visualmente los coeficientes $\tau_4$ con y sin JP rezagada.

```{r}
#| label: fig-mediacion
#| fig-cap: "Coeficiente DiD (Ola 4 × Indígena × Zona) con y sin JP rezagada (ola 3). IC 95%. Ajuste exploratorio, no mediación causal."
#| fig-width: 10
#| fig-height: 8
#| out-width: "95%"
...
Terciles = exploratorio (sin corrección multiplicidad). El decreto opera por proximidad territorial $Z$ más que por intensidad identitaria lineal (A8). Plebiscito 2022 = asociacional (ola 4; @sec-apendice-a10).
